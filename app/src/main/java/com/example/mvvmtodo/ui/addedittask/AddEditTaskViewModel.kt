...
class AddEditTaskViewModel @ViewModelInject constructor(
    ...
    fun onSaveClick() {
        ...
        if (taskName.isBlank()) {
          showInvalidInputMessage("TasQ cannot be Empty")
            return
        }

        if (task != null) {
            val updatedTask = task.copy(name = taskName, important = taskImportance, false)
            updateTask(updatedTask)
        } else {
            val newTask = Task(name = taskName, important = taskImportance, false)
            createTask(newTask)
        }
